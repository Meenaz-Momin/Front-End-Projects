...
class deleteStandardResponse(unittest.TestCase):
    def setUp(self):
        self.driver = webdriver.Chrome()

    def test_deleteStandardResponse(self):
        driver = self.driver
        try:
            ...
            try:
                ...
                delete = driver.find_element(
                    By.XPATH, "//*[@id='__next']/div[1]/main/div[2]/div[2]/div[2]/div/div[2]/div[2]/div/div/div/div[1]/div[3]/div/button[2]")
                delete.click()
                time.sleep(2)
                ...
                if(rows2 < rows):
                    file = open("Manager_Report.txt", "a")
                    file.write(
                        "\nDelete Standard Response: Pass\t Date:")
                    file.write(dt_string)
                    file.close()
                    ...
                    file = open("Manager_Report.txt", "a")
                    file.write(
                        "\nDelete Standard Response: Fail\t Date:")
                    file.write(dt_string)
                    file.close()
