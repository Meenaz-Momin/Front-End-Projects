        ...
        try:
            ...
            try:
                ...
                if(rows == 0):
                    file = open("Manager_Report.txt", "a")
                    file.write(
                        "\nDelete Questionnaire: No List\t Date:")
                    file.write(dt_string)
                    file.close()
                else:
                    delete = driver.find_element(
                        By.XPATH, "//*[@id='__next']/div[1]/main/div[2]/div[2]/div[2]/div/div[2]/div[2]/div/div/div/div[1]/div[5]/div/button[2]")
                    delete.click()
                    time.sleep(2)
                    ...
                    if(rows2 < rows):
                        file = open("Manager_Report.txt", "a")
                        file.write(
                            "\nDelete Questionnaire: Pass\t Date:")
                        file.write(dt_string)
                        file.close()
